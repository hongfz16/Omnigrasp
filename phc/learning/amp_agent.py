...
class AMPAgent(common_agent.CommonAgent):
    ...
    def __init__(self, base_name, config):
        ...
        if self.vec_env.env.task.fitting:
            ...
            if len(self.vec_env.env.task.models_path) > 0:
                print("#################### Loading model!! ####################")
    ...
    def pre_epoch(self, epoch_num):
        ...
        if self.vec_env.env.task.humanoid_type in ["smpl", "smplh", "smplx"]:
            ...
            if humanoid_env.getup_schedule and "update_getup_schedule" in humanoid_env.__dir__():
    ...
    def _optimize_kin(self, batch_dict):
        ...
        if humanoid_env.distill: 
            ...
            if humanoid_env.z_type == "vae":
                ...
                if humanoid_env.use_part:
                    kin_action_loss_body = torch.norm(pred_action[:, humanoid_env.body_name_idxes] - gt_action[:, humanoid_env.body_name_idxes], dim=-1).mean()  ## RMSE
                    kin_action_loss_left = torch.norm(pred_action[:, humanoid_env.left_dof_idxes] - gt_action[:, humanoid_env.left_dof_idxes], dim=-1).mean()  ## RMSE
                    kin_action_loss_right = torch.norm(pred_action[:, humanoid_env.right_dof_idxes] - gt_action[:, humanoid_env.right_dof_idxes], dim=-1).mean()  ## RMSE
                    kin_action_loss = kin_action_loss_body * 1.5 + kin_action_loss_left * 0.5 + kin_action_loss_right * 0.5
                else:
                ...
                ######### KLD annealing #######
                if humanoid_env.kld_reverse_anneal:
                    anneal_start_epoch = 2500
                    anneal_end_epoch = 5000
                    min_val = humanoid_env.kld_coefficient_min
                    max_val = humanoid_env.kld_coefficient_max
                    humanoid_env.kld_coefficient = min_val + (max_val - min_val) * min(max((self.epoch_num - anneal_start_epoch) / (anneal_end_epoch - anneal_start_epoch), 0) , 1)
                    info_dict["kin_kld_w"] = humanoid_env.kld_coefficient
                ######### KLD annealing #######
            else:
                raise NotImplementedError()    
