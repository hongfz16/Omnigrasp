...
class HumanoidOmniGrasp(humanoid_amp_task.HumanoidAMPTask):
    ...
    def _compute_task_obs(self, env_ids=None):
        ...
        obs = None
        if self.cfg.env.get("use_grab_obs", False):
            if self.obs_v == 1:
        ...
        if self.cfg.env.get("use_track_obs", False):
            motion_res = self._get_state_from_motionlib_cache(motion_ids_steps, motion_times_steps) #, self._global_offset[env_ids].repeat_interleave(time_steps, dim=0).view(-1, 3))  # pass in the env_ids such that the motion is in synced.
            ...
            track_obs = compute_imitation_observations(
                root_pos, root_rot, body_pos_subset, body_rot_subset, body_vel_subset, body_ang_vel_subset, ref_rb_pos_subset, ref_rb_rot_subset, ref_body_vel_subset, ref_body_ang_vel_subset, time_steps, self._has_upright_start
            )
            if obs is None:
                obs = track_obs
            else:
                obs = torch.cat([obs, track_obs], -1)
    ...
    def _compute_reward(self, actions):
        ...
        if self.cfg.env.get("use_track_reward", False):
            ...
            if self.reward_raw is not None:
                ...
                self.reward_raw = track_reward_raw
    ...
    def _compute_reset(self):
        ...
        if self.cfg.env.get("use_track_reset", False):
            ...
            other_distance = 0.25 #0.25 #0.2
            foot_distance = 0.5 #0.25
            wrist_distance = 0.15 #0.2 #0.1
            finger_termination_distance = 0.15 #0.2 #0.1
            if self._track_body_ids.shape[0] == 15:
                termination_distances = [other_distance, foot_distance, foot_distance, wrist_distance, wrist_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance, finger_termination_distance]
...
def compute_grab_reward(root_pos, root_rot, obj_pos, obj_rot, obj_vel, obj_ang_vel,  ref_obj_pos, ref_obj_rot, ref_body_vel, ref_body_ang_vel, contact_filter, curr_contact_obs, rwd_specs):
    ...
    # print("grab_reward", r_obj_pos, r_obj_rot, r_lin_vel, r_ang_vel, contact_filter, r_contact_lifted, contact_symbol, (w_pos * r_obj_pos + w_rot * r_obj_rot + w_vel * r_lin_vel + w_ang_vel * r_ang_vel) + r_contact_lifted * w_conctact * contact_symbol, reward)
...
def compute_imitation_reward(body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, rwd_specs):
    ...
    if ref_body_pos.shape[1]==15:
        ...
        diff_global_body_pos = ref_body_pos[:,:1] - body_pos[:,:3]
        diff_body_pos_dist = (diff_global_body_pos**2).mean(dim=-1).mean(dim=-1)

        diff_global_body_pos_hand = ref_body_pos[:,1:] - body_pos[:,1:]
