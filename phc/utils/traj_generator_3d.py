...
class TrajGenerator3DInFile():

    def __init__(self, num_envs, episode_dur, num_verts, device, traj_config, pkl_data, starting_still_dt=1.5):
        ...
        data_keys = [k for k in pkl_data.keys()][0]
        self.obj_pose = torch.from_numpy(pkl_data[data_keys]['obj_data']['obj_pose'].reshape(-1, 2, 7)[:, 0]).float().to(self._device)
    ...
    def reset(self, env_ids, init_pos, init_rot, time_till_table_removal = 1.5):
        ...
        #         vert_pos[..., 2] = torch.clamp(vert_pos[..., 2], 0.3, 1.8)
            ...
            self.gts[env_ids] = self.obj_pose[None, :num_verts, :3]
            self.grs[env_ids] = self.obj_pose[None, :num_verts, 3:]
